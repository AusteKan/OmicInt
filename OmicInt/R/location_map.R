#' @title location_map
#'
#' @description Function provides visualisation of how the highest and lowest LFCscore  genes cluster based on protein cellular location data.
#'
#' @param  data Requires a  data frame generated by score_genes
#' @param  num a number for genes to cluster from top upregulated and downregulated genes, if not selected all genes will be used; default 0, i.e. do not select a smaller set
#' @return dendogram
...
location_map<-function(data, num=0){
  ...
  graphics::par(mar = c(8,3,3,3))
  plot(dendextend::color_branches(dend,  h= mean(h_clust$height)*dend_k$nc*2) )
  dendextend::colored_bars(colors =as.vector(col_vector_names), dend = dend,add = TRUE, rowLabels = "Location")
  graphics::legend("topright", legend = names(colors), fill = as.character(colors),xpd=TRUE,  cex=0.7, bty='n', title = "Cellular location")
}
